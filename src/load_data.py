...
def _create_ingredient_dict(menu_ingre_df: pd.DataFrame, ingre_price_df: pd.DataFrame) -> Dict[str, List[Ingredient]]:
    ...
    for _, row in menu_ingre_df.iterrows():
        ...
        price_series = ingre_price_df[ingre_price_df['Ingredient'] == ingredient_name]['단가(원/g)']
        if not price_series.empty and not pd.isna(price_series.values[0]):
            price = float(price_series.values[0]) * amount
        else:
            price = 0
